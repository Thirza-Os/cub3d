...
/*                                                   +#+                      */
/*   Created: 2024/08/09 01:14:13 by lvan-gef      #+#    #+#                 */
/*   Updated: 2024/08/09 01:19:59 by lvan-gef      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void draw_info_calc(t_dda *dda) {
    if (dda->current_side == X_SIDE) {
        dda->perp_wall_dist = (dda->map_pos.col - dda->player_pos.col + (1 - dda->step_map.col) / 2) / dda->ray_dir.col;
    } else {
        dda->perp_wall_dist = (dda->map_pos.row - dda->player_pos.row + (1 - dda->step_map.row) / 2) / dda->ray_dir.row;
    }
